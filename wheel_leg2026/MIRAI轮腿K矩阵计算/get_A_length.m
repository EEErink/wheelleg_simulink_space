...
function A = get_A_length(leg_length)

   syms x(t) T R Iw mw M L LM theta(t) l phi(t) mp g Tp Ip IM
    syms f1 f2 f3 d_theta d_x d_phi theta0 x0 phi0 

    R1=0.063;                           %驱动轮半径
    L1=leg_length*0.458 + 0.079387;      %摆杆重心到驱动轮轴距离
    LM1=leg_length - L1;                 %摆杆重心到其转轴距离
    l1=0.005;                          %机体质心距离转轴距离
    mw1=0.60;%0.54                         %驱动轮质量
    mp1=1.71;                         %杆质量
    M1=17;                            %机体质量
    Iw1=0.18*R1^2 +1.44*0.04^2;       %驱动轮转动惯量
    Ip1=0.1145*leg_length+0.019557;               %摆杆转动惯量
    IM1=M1*(0.3^2+0.12^2)/2/12.0;       %机体绕质心转动惯量
