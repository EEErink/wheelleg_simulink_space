...
R_l_ac = 0.23;                            % 两个驱动轮之间距离/2         （单位：m）
l_c_ac = -0.001;                            % 机体质心到腿部关节中心点距离  （单位：m）
m_w_ac = 0.632; m_l_ac = 0.996; m_b_ac = 22.42;    % 驱动轮质量 腿部质量 机体质量  （单位：kg）
I_w_ac = 0.0004986;                            % 驱动轮转动惯量               （单位：kg m^2）
I_b_ac = 0.4066;                            % 机体转动惯量(自然坐标系法向)  （单位：kg m^2）
I_z_ac = 0.675;                            % 机器人z轴转动惯量            （单位：kg m^2）
...
l_wl_ac = 0.4986*0.15 + 0.0553;       % 左驱动轮质心到左腿摆杆质心距离                      （单位：m）
l_bl_ac = 0.1347*0.15 + 0.0576;       % 机体转轴到左腿摆杆质心距离                          （单位：m）
I_ll_ac = 0.0229*0.15 + 0.0107;      % 左腿摆杆转动惯量                                   （单位：kg m^2）
l_r_ac = 0.15;        % 右腿摆杆长度                      （右腿对应数据）  （单位：m）
l_wr_ac = 0.4986*0.15 + 0.0553;        % 右驱动轮质心到右腿摆杆质心距离                      （单位：m）
l_br_ac = 0.1347*0.15 + 0.0576;       % 机体转轴到右腿摆杆质心距离                          （单位：m）
I_lr_ac = 0.0229*0.15 + 0.0107;      % 右腿摆杆转动惯量                                   （单位：kg m^2）
...
% 
Leg_data_l = [0.11,  0.4986*0.11 + 0.0553,   0.1347*0.11 + 0.0576,  0.0229*0.11 + 0.0107;
...
% Leg_data_l = [
%     0.11,  0.11 - (0.4986*0.11 - 0.0553),          0.4986*0.11 - 0.0553,        0.0229*0.11 + 0.0107;
...
% % LQR 权重
% lqr_Q = diag([5, 100, 60, 20, 500, 50, 500,50, 4000, 100]);
% lqr_R = diag([10, 10, 1, 1]);
% lqr_Q = diag([5, 100, 60, 20, 500, 50, 500,50, 4000, 100]);
% lqr_R = diag([1, 1, 0.1, 0.1]);
...
% - 第37行对应K_4,7
% ... 其他行对应关系类似
 %p00 + p10*l_l + p01*l_r + p20*l_l^2 + p11*l_l*l_r + p02*l_r^2
...
K_Fit_Coefficients = sprintf(['{' strjoin(repmat({'%.5g'},1,6),', ') '},\n'], K_Fit_Coefficients.')
% 正确食用方法：
